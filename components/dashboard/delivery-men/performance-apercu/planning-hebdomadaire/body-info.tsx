...
export default function BodyInfo({ data }: { data: PerformanceHebdomadaire }) {
  ...
  const fnData=()=>{
    ...
    const mois = data.creneauVM.debut.substring(5, 7);
    const jourDebut = data.creneauVM.debut.substring(8, 10);
    const jourFin = data.creneauVM.fin.substring(8, 10);
    ...
    return  `Créneau ${jourDebut}-${jourFin} ${moi}`
}
  ...
  return (
       <div className="py-10">
              <div className=' gap-2 lg:gap-0 px-2 lg:px-4 flex items-center justify-between'>
                <div >Aujourd'huit {today}</div>
                <div className='bg-red-400 rounded-lg p-2'>{fnData()}</div>
                <div>-</div>
              </div>
              ...
              <div className='flex flex-col gap-4 mt-8'>
                {data.jours.map((item, index) => {
                  ...
                  return (
                    <div className="flex border-2 py-5 px-2 rounded-lg ">
                      <div className="pr-2"> <span>{item.jour}</span> <span>...</span></div>
