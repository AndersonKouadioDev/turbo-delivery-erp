...
import { getRestaurantDefined, getRestaurantUndefined } from '@/src/price-list/price-list.action';
...
export default function Header({ initialData }: { initialData: Restaurant[]|null}) {
    ...
    useEffect(()=>{
        ...
        const restaurantDetails=initialData?.filter(item=>item.id===restaurantId)
        ...
        setInputCalculate(prevState => ({
            ...
            point1: { lat: restaurantDetails[0].latitude || 0, lng:restaurantDetails[0].longitude || 0 }
            }));
        }
    },[restaurantId])
    ...
    const handleSuggestionClick = async (suggestion: PlaceAutocompleteResult) => {
        ...
        try {
            ...
            setResulFinaleDistance(calculateDistanceResult) 
